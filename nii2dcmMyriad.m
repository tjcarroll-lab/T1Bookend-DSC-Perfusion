function nii2dcmMyriad(bpnii,bpdcm,bphdr)
%nii2dcmMyriad Convert Nifti files to dicom for motion corrected MYRIAD
...
for s = 1:size(data, 4) % By slice
    for t = 1:size(data, 3) % By time
        count = count + 1;
        dicomwrite(data(:,:,t,s), [bpdcm '\' num2str(count) '.dcm'], hdrs{count}, 'CreateMode', 'copy', 'WritePrivate', true);
    end
end
