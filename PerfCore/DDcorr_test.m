function [DDfit] = DDcorr_test(AIF,Dt,Wij,VEIN,path_DSC)
...
%     VEIN.FitParam = FitParam;
    ...
    vtime = 0:Dt:(VEIN.RTP-VEIN.BATP)*Dt;
    vA = VEIN.FitParam.A;
    valpha = VEIN.FitParam.alpha;
    vbeta = VEIN.FitParam.beta;
    ...
    figure;plot(AIF.Ct);hold on;plot(VEIN.Ct);
    % set a threshold to cut out front-end zeros (e.g. inaccurate BAT)
    [~,nmax] = max(AIF.Ct);
    %cutoff = .015 * max( max(AIF.Ct), max(VEIN.Ct) ); % YIJ 20200221: used to be 0.0001
    %cutoffind = find(AIF.Ct(1:nmax) > cutoff,1);
    cutoffind = find(diff(AIF.Ct) > .2 * max(diff(AIF.Ct)),1) + 1;
    fprintf('AIF: %d (%d) ',cutoffind,AIF.BATP);
%     if cutoffind > 1
...
%     end
    ...
    [~,nmax] = max(VEIN.Ct);
    %cutoffind = find(VEIN.Ct(1:nmax) > cutoff,1);
    cutoffind = find(diff(VEIN.Ct) > .2 * max(diff(VEIN.Ct)),1) + 1;
    fprintf('VEIN: %d (%d)\n',cutoffind,VEIN.BATP);
%     if cutoffind > 1
...
%     end
    ...
    input_AIF = [AIF.Ct gamma_variate_model_IDC(atime(end)+Dt:Dt:atime(end) + addaif*Dt,aA,aalpha,abeta)];
    ...
    try
    [fit0,gof0,output0] = fit(time',Residue',fiteq,'problem',ATD,'StartPoint',[1 1]);
    catch
       fit0 = []; 
    end
    DDfit = fit0;
    ...
    title(sprintf('rsquared = %f',gof0.rsquare));
    
    %exportgraphics(gca, ['..\images\autovein debug\' currtime '_' caseid '_ddfit.jpg'], 'resolution', 300);
    end
