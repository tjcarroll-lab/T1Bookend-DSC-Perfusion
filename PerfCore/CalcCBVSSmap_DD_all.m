function [CBVssFast_per, CBVssSlow_per,T1s] = CalcCBVSSmap_DD_all(images,position,wmmask)

% predefine
t0 = 800;

% prepare
temp = images.t1.T1map_pre;
mask = ones(size(temp));
mask(find(temp>=2000))=0;mask(find(temp<=0))=0;

T1map_pre   = fSpatialFilter(images.t1.T1map_pre,[1 1],mask);
M0map_pre   = fSpatialFilter(images.t1.M0map_pre,[1 1],mask);
InvFmap_pre = fSpatialFilter(images.t1.InvFmap_pre,[1 1],mask);
T1map_post  = fSpatialFilter(images.t1.T1map_post,[1 1],mask);
M0map_post  = fSpatialFilter(images.t1.M0map_post,[1 1],mask);
InvFmap_post= fSpatialFilter(images.t1.InvFmap_post,[1 1],mask);
