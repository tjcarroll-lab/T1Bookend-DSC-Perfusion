function [Signal_AIF,cutoffs_AIF,position,Conct_AIF,Images,n_AIFslice,AIF,positionAIF] = autoaif_wy_Philips_v2(path_DSC,N_Slices,N_meas,tmpdata)
...
for s = 1:N_Slices
    ...
    if mask_s(i,j)
        ...
        tmpary = reshape(smooth(images(i,j,:),5,'sgolay',1),[1 size(images,3)]);
...
[iBAT BAT BRT] = findBAT(Mean_Signal);
if(iBAT == 0)
    iBAT = 1;
end
...
%YIJ 20180130
leakratio = MeanS0/mean(Mean_Signal(BRT+1:end));
...
for s = 1:N_Slices
    ...
    %mask_AIF = mask_all{s};
    %figure(); imshow(images(:,:,2), []); title('Make mask of brain'); mask_AIF = roipoly;
...
%     end
    ...
    for j = 1:jsize
        ...
        if mask_AIF(i,j)
            ...
            temp2 = reshape(smooth(images(i,j,:),5,'sgolay',1),[1 TimePoint]);
...
%             [ibat2 bat2 brt2] = findBAT(tmptmp2);
            ...
            %YIJ 20180130 adaptive leakage ratio
            if meanS0 > leakratio*mean(temp(BRT+1:end))% - 1.96*std(temp(BRT+1:end))
            ...
            %if meanS0 < 400
            %YIJ 20180130 adaptive low-end threshold
            if meanS0 < MeanS0
                mask_AIF(i,j) = 0;
            end
...
try
    ...
    tmptemp = freqfilter(temp,'low',55);
...
for i = 1:size(images,1)
    for j = 1:size(images,2)
        if Mask_AIF_slice(i,j,n_AIFslice)
            ...
            Signal_AIF2 = Signal_AIF2 + reshape(smooth(Images{n_AIFslice}(i,j,:),5,'sgolay',1),[1 TimePoint]);
    ...
    for i = 1:size(images,1)
        for j = 1:size(images,2)
            if Mask_AIF(i,j,s)
                ...
                Signal_AIF2 = Signal_AIF2 + reshape(smooth(Images{s}(i,j,:),5,'sgolay',1),[1 TimePoint]);
