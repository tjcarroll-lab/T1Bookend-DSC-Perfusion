function Auto_qCBF_Philips_ManualWMmask_MSwcf2_tmp(varargin)
%-------------------------------------------------------------------------%
    ...
    if namefindinstrcut(Seqfiles,'ep2d_perf')
        ...
        if namefindinstrcut(AIFfiles,targetAIFfile,'exact') & namefindinstrcut(DSCfiles,targetDSCfile,'exact')
            ...
            if strmatch(option,'Overwrite')
                ...
                [CBV_DSC,CBF_nSVD,CMTT_nSVD,Tmax_map,dBATmap,Conc_map,CmtWM] = Calc_nSVD_Philips(targetpathDSC,N_images,ROIs.data.AIF,cutoffs_ROIs.AIF,N_meas,ROIs.data.AIFslice,ROIs.positions.n_slice_AIF);
                cutoffs_ROIs.CmtWM = CmtWM;
                [images, image_names] = IA_ImportImages(images, image_names,'CBV_DSC',CBV_DSC,'CBF_nSVD',CBF_nSVD,'CMTT_nSVD',CMTT_nSVD,'TmaxMap_nSVD',Tmax_map,'dBATmap',dBATmap,'Conc_map',Conc_map);
        ...
        elseif namefindinstrcut(AIFfiles,targetAIFfile,'exact')
            ...
            [CBV_DSC,CBF_nSVD,CMTT_nSVD,Tmax_map,TTP_map,dBATmap,Conc_map,CmtWM,ATDmap,IBATmap,BATmap,BRTmap,sigmap,VEINslice,veinmask,positionVein,n_Veinslice] = Calc_nSVD_Philips(targetpathDSC,N_images,ROIs.data.AIF,cutoffs_ROIs.AIF,N_meas,ROIs.data.AIFslice,ROIs.positions.n_slice_AIF);
            % -------------------------------------------------------------
            cutoffs_ROIs.CmtWM = CmtWM;
            ROIs.sigmap = sigmap;
            ...
            % Mira change this code----------------------------------------
            [images, image_names] = IA_ImportImages(images, image_names,'CBV_DSC',CBV_DSC,'CBF_nSVD',CBF_nSVD,'CMTT_nSVD',CMTT_nSVD,'Tmax_map',Tmax_map,'TTP_map',TTP_map,'dBATmap',dBATmap,'Conc_map',Conc_map,'ATDmap',ATDmap,'IBATmap',IBATmap,'BATmap',BATmap,'BRTmap',BRTmap);
            % -------------------------------------------------------------
    ...
    if namefindinstrcut(Seqfiles,'ep2d_se')
        ...
        if namefindinstrcut(AIFfiles,targetAIFfile,'exact') & namefindinstrcut(DSCfiles,targetDSCfile,'exact')
            ...
            if strmatch(option,'Overwrite')
                ...
                %[DSC,headers,N_images] = ReadDICOMSeries(targetpathDSC,1);
                N_images = N_slices;
                [CBV_DSC,CBF_nSVD,CMTT_nSVD,Tmax_map,dBATmap,Conc_map,CmtWM] = Calc_nSVD_Philips(targetpathDSC,N_images,ROIs.data.AIF,cutoffs_ROIs.AIF,N_meas);
                cutoffs_ROIs.CmtWM = CmtWM;
                [images, image_names] = IA_ImportImages(images, image_names,'CBV_DSC',CBV_DSC,'CBF_nSVD',CBF_nSVD,'CMTT_nSVD',CMTT_nSVD,'TmaxMap_nSVD',Tmax_map,'dBATmap',dBATmap,'Conc_map',Conc_map);
                save([targetpath '/DSCanalysis/' targetDSCfile], 'ROIs','cutoffs_ROIs','images', 'image_names');
                %%
