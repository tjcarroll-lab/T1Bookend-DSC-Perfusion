function [Vein,Mask_Vein_slice,positionVein,n_Veinslice] = autoVein(path_DSC,N_meas,N_Slices,AIFslice,caseid)
...
for s = 1:N_Slices
    ...
    if mask_s(i,j)
        ...
        tmpary = reshape(smooth(images(i,j,:),5,'sgolay',3),[1 size(images,3)]);
        sum_signal = sum_signal+ tmpary;
        
        % 20210210 YIJ: collect list of bat, brt, and nmin
        [~,tmpbat,tmpbrt] = findBAT(tmpary);
        [~,tmpnmin] = min(tmpary);
...
%     end
    ...
    for j = 1:jsize
        ...
        if mask_Vein(i,j)
            ...
            tempsig2 = reshape(smooth(images(i,j,:),max(.05*TimePoint,5),'sgolay',3),[1 TimePoint]);
    ...
    for i = 1:size(images,1)
        for j = 1:size(images,2)
            if Mask_Vein_slice(i,j,n_Veinslice)
                ...
                Signal_Vein2 = Signal_Vein2 + reshape(smooth(Images{n_Veinslice}(i,j,:),max(.05*TimePoint,5),'sgolay',3),[1 TimePoint]);
...
[ibat, bat, brt] = findBAT(Signal_Vein2);
...
title(['vein: ' num2str(bat) '-' num2str(brt) '-s' num2str(positionVein{1}(3))]);
subplot(2,2,3);
plot(Mean_Signal);
subplot(2,2,4);
