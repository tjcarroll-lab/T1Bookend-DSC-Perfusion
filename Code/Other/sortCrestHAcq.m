function [copypath] = sortCrestHAcq(bpcase)
%sortCrestHAcq Reorders perfusion series for compatibility
...
%     copyto = fullfile(bpcase,[seriesname ' FE_EPI'],[seriesname ' FE_EPI_' num2str(ii) '.dcm']);
    if ~exist([bpcase '_FE_EPI'], 'dir')
        mkdir([bpcase '_FE_EPI']);
    end
    copyto = fullfile([bpcase '_FE_EPI'],['FE_EPI_' num2str(ii) '.dcm']);
    
    movefile(copyfrom,copyto);
end

copypath = [bpcase '_FE_EPI'];
end
