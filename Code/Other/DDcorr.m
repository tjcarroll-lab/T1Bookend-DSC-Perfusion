function [DDfit,VEIN] = DDcorr(AIF,Dt,echoTime,Wij,path_DSC,Nmeas,Nslices)
...
if exist([glblTargetPath '\Vein_Mask_P' sprintf('%03d',injectionNum) seqType '.mat'],'file')
    ...
    AIF.Ct = AIF.Ct(cutoffind:end);
    AIF.BATP = AIF.BATP + cutoffind - 1;
    
    [~,nmax] = max(VEIN.Ct);
    cutoffind = find(VEIN.Ct(1:nmax) > 0.0001,1);
    VEIN.Ct = VEIN.Ct(cutoffind:end);
    VEIN.BATP = VEIN.BATP + cutoffind - 1;
