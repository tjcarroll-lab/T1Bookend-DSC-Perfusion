function [CBVssFast_per, CBVssSlow_per,T1s] = CalcCBVSSmap2(images,image_names,position,wmmask)

global glblTargetPath;
global seqType;
tmp = load([glblTargetPath '\Result_MSwcf2\P001' seqType '.mat']);
images1 = tmp.images;
position1 = tmp.ROIs.positions;

% predefine
t0 = 800;

% prepare
temp = images1{strmatch('T1map_pre',image_names)};
mask = ones(size(temp));
mask(find(temp>=2000))=0;mask(find(temp<=0))=0;

T1map_pre   = fSpatialFilter(images1{strmatch('T1map_pre',image_names,'exact')},[1 1],mask);
M0map_pre   = fSpatialFilter(images1{strmatch('M0map_pre',image_names,'exact')},[1 1],mask);
InvFmap_pre = fSpatialFilter(images1{strmatch('InvFmap_pre',image_names,'exact')},[1 1],mask);
...
if isfield(position1,'blood_pre') & isfield(position,'blood_post')
    T1_blood_pre = 0;    M0_blood_pre = 0;   InvF_blood_pre = 0;    tempcount = 0;
    T1_blood_post = 0;   M0_blood_post = 0;  InvF_blood_post = 0;
    for i = 1:length(position1.blood_pre)
        if (T1map_pre(position1.blood_pre{i}(1),position1.blood_pre{i}(2)) ~= 0) & (T1map_pre(position1.blood_pre{i}(1),position1.blood_pre{i}(2)) < 3000) & isfinite(T1map_pre(position1.blood_pre{i}(1),position1.blood_pre{i}(2)))
            T1_blood_pre = T1_blood_pre + T1map_pre(position1.blood_pre{i}(1),position1.blood_pre{i}(2));
            tempcount = tempcount + 1;
        end
        M0_blood_pre = M0_blood_pre + M0map_pre(position1.blood_pre{i}(1),position1.blood_pre{i}(2));
        InvF_blood_pre = InvF_blood_pre + InvFmap_pre(position1.blood_pre{i}(1),position1.blood_pre{i}(2));
    end
    T1_blood_pre = T1_blood_pre/tempcount; 
    M0_blood_pre = M0_blood_pre/length(position1.blood_pre);
    InvF_blood_pre = InvF_blood_pre/length(position1.blood_pre);  
