function Auto_qCBF_Philips_DD_all(varargin)
%-------------------------------------------------------------------------%
...
if nargin
    ...
    tmppathsplit = split(targetpath,{'\','/'});
    caseid = strcat(tmppathsplit{end-2},'-',tmppathsplit{end-1});
    ...
    for ii = 1:length(input)
        ...
        if sum(strcmpi(PN,{'P001'}))
        %if sum(strcmpi(PN,{'P001','P002','P003','P004','P005'}))
...
for j = 1:length(Seqfiles);
    ...
    if ~isempty(strfind(temp.name,'LL_EPI_PRE')) |  ~isempty(strfind(temp.name,'LL_EPI_pre'))
        if isempty(strfind(temp.name,'LL_EPI_PRE2'))
            ...
            t1.T1map_pre = T1map_pre;
            t1.M0map_pre = M0map_pre;
            t1.InvFmap_pre = InvFmap_pre;
            t1.RESNORMmap_pre = RESNORMmap_pre;
    ...
    if  ~isempty(strfind(temp.name,'LL_EPI_POST')) |  ~isempty(strfind(temp.name,'LL_EPI_post'))
        if isempty(strfind(temp.name,'LL_EPI_POST2'))
            ...
            t1.T1map_post = T1map_post;
            t1.M0map_post = M0map_post;
            t1.InvFmap_post = InvFmap_post;
            t1.RESNORMmap_post = RESNORMmap_post;
            count = count+4; index=1;
        end
    end
end
if index
    images.t1 = t1;
    save([targetpath '/T1mapping/' PN '_T1map.mat'], 'images');
...
if ~isempty(images.t1.T1map_pre) & ~isempty(images.t1.M0map_pre) & ~isempty(images.t1.T1map_post) & ~isempty(images.t1.M0map_post)
    T1map_pre  = images.t1.T1map_pre;
    T1map_post = images.t1.T1map_post;
    M0map_pre  = images.t1.M0map_pre;
    M0map_post = images.t1.M0map_post;
    RESNORMmap_pre = images.t1.RESNORMmap_pre;
    RESNORMmap_post = images.t1.RESNORMmap_post;
    ...
    save([targetpath '/AutoSS/' targetT1file(1:5) 'AutoSS.mat'], 'ROIs', 'masks_ROIs','images')
end
...
if strmatch(Seqtarget,'GE')
    ...
    if namefindinstrcut(Seqfiles,'ep2d_perf')
        ...
        elseif namefindinstrcut(AIFfiles,targetAIFfile,'exact')
            ...
            %            N_images = 15;
            N_images = N_slices; %Jessy (10/31/2008) %25 is correct Grady
            [noDD,DD,CmtWM,etc,sigmap,VEINslice,veinmask,positionVein,n_Veinslice] = Calc_nSVD_Philips_DD_all(targetpathDSC,N_images,ROIs.data.AIF,cutoffs_ROIs.AIF,N_meas,ROIs.data.AIFslice,ROIs.positions.n_slice_AIF);
            cutoffs_ROIs.CmtWM = CmtWM;
            ROIs.sigmap = sigmap;
            ...
            save([targetpath '/DSCanalysis/' targetDSCfile], 'ROIs','cutoffs_ROIs','images','masks_ROIs');
            %%
        end
    end
elseif strmatch(Seqtarget,'SE')
...
images = OverloadStructdata(tempimages,images);
...
% -------------------------------------------------------------------------
[masks_ROIs.WM_SS,cutoffs_ROIs.WM_SS,ROIs.data.distr.T1s,ROIs.data.distr.XT1s] = fAutoMaskWM_SS(images.t1.T1map_pre,ROIs.values.Tesla);
...
if exist([targetpath '/WM_Mask_' targetresultfile], 'file') == 2
    ...
    masks_ROIs.WM_SS = fManualMaskWM_SS(images.t1.T1map_pre,images.t1.T1map_post);
...
tempCBV = images.noDD.CBV_DSC(:,:,nslice);
...
%tempCBVwm(tempCBVwm < 1) = [];
[ROIs.data.distr.noDD.CBVdsc edges] = histcounts(tempCBVwm,'binmethod','fd');
%[ROIs.data.distr.CBVdsc edges] = histcounts(tempCBVwm,'binmethod','fd');
width = edges(2) - edges(1);
ROIs.data.distr.noDD.XCBVdsc = edges(1:end-1) + width/2;

cutoffs_ROIs.noDD.CBVdsc = zeros(1,2);
temp = find(ROIs.data.distr.noDD.CBVdsc);
cutoffs_ROIs.noDD.CBVdsc(1) = ROIs.data.distr.noDD.XCBVdsc(min(temp));
cutoffs_ROIs.noDD.CBVdsc(2) = ROIs.data.distr.noDD.XCBVdsc(max(temp));

% fit CBVdsc distribution
ROIs.data.fitting.noDD.CBVdsc = CalfitCBVdsc(ROIs.data.distr.noDD.XCBVdsc,ROIs.data.distr.noDD.CBVdsc,cutoffs_ROIs.noDD.CBVdsc,PN);
CBVdsc_WM = ROIs.data.fitting.noDD.CBVdsc.b1;

% DD--------------------------------------------
if DDfix
    % find cutoffs in cbvdsc distribution
    tempCBV = images.DD.CBV_DSC(:,:,nslice);
    ...
    % YIJ 20200213: only count CBVdsc > 1, because less than 1 (small numbers like in csf) can bias
    %tempCBVwm(tempCBVwm < 1) = [];
    [ROIs.data.distr.DD.CBVdsc edges] = histcounts(tempCBVwm,'binmethod','fd');
    %[ROIs.data.distr.CBVdsc edges] = histcounts(tempCBVwm,'binmethod','fd');
    width = edges(2) - edges(1);
    ROIs.data.distr.DD.XCBVdsc = edges(1:end-1) + width/2;
    
    cutoffs_ROIs.DD.CBVdsc = zeros(1,2);
    temp = find(ROIs.data.distr.DD.CBVdsc);
    cutoffs_ROIs.DD.CBVdsc(1) = ROIs.data.distr.DD.XCBVdsc(min(temp));
    cutoffs_ROIs.DD.CBVdsc(2) = ROIs.data.distr.DD.XCBVdsc(max(temp));
    
    % fit CBVdsc distribution
    ROIs.data.fitting.DD.CBVdsc = CalfitCBVdsc(ROIs.data.distr.DD.XCBVdsc,ROIs.data.distr.DD.CBVdsc,cutoffs_ROIs.DD.CBVdsc,PN);
    CBVdsc_WM = ROIs.data.fitting.DD.CBVdsc.b1;
end
...
switch injectionNum
    case 1
        [CBVssFast_per,CBVssSlow_per,T1s] = CalcCBVSSmap_DD_all(images,ROIs.positions,masks_ROIs.WM_SS);
    case 2
        [CBVssFast_per,CBVssSlow_per,T1s] = CalcCBVSSmap_DD_all(images,ROIs.positions,masks_ROIs.WM_SS);
    otherwise
        [CBVssFast_per,CBVssSlow_per,T1s] = CalcCBVSSmap_DD_all(images,ROIs.positions,masks_ROIs.WM_SS);
...
% quantification
[ROIs.values.CF,images.noDD.qCBF_SVD]   = Calc_QCBV_CBF_MS2_DD_all(images.noDD.CBF_SVD,ROIs.values.T1s,ROIs.data.fitting.CBVssFast,ROIs.data.fitting.noDD.CBVdsc,ROIs.values.Tesla);
[ROIs.values.CF,images.noDD.qCBV_DSC]   = Calc_QCBV_CBF_MS2_DD_all(images.noDD.CBV_DSC,ROIs.values.T1s,ROIs.data.fitting.CBVssFast,ROIs.data.fitting.noDD.CBVdsc,ROIs.values.Tesla);
ROIs.values.CF.ratio_CBV_noDD = ROIs.values.CF.ratio_CBV;
if DDfix
    [tmpCF,images.DD.qCBF_SVD]   = Calc_QCBV_CBF_MS2_DD_all(images.DD.CBF_SVD,ROIs.values.T1s,ROIs.data.fitting.CBVssFast,ROIs.data.fitting.DD.CBVdsc,ROIs.values.Tesla);
    [tmpCF,images.DD.qCBV_DSC]   = Calc_QCBV_CBF_MS2_DD_all(images.DD.CBV_DSC,ROIs.values.T1s,ROIs.data.fitting.CBVssFast,ROIs.data.fitting.DD.CBVdsc,ROIs.values.Tesla);
    ROIs.values.CF.ratio_CBV_DD = tmpCF.ratio_CBV;
end
...
save([targetpath '/Result_MSwcf2/' targetresultfile],'ROIs','cutoffs_ROIs','images','masks_ROIs');
